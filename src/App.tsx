    ...
    <main className="ax-page">
      <div className="wrap">
        <header className="masthead">
          <span className="mh-kicker"><span className="dot" />行程作战表 · Travel Ops</span>
          <h1 className="mh-title">Anime Expo <span className="yr">2026</span></h1>
          <p className="mh-sub">
        ...
        <section className="nextup" aria-label="下一步行动">
          ...
          <div className="nu-body">
            ...
            <div className="nu-note">{nextItem.parkingPrimary} · {statusLabel[nextItem.status]}</div>
          </div>
        </section>
          ...
          <section className={`day accent-${day.accent}`} key={day.id}>
            ...
            <div className="timeline">
              {tripItems
                ...
                .map((item) => (
                  <TicketStop item={item} stopNumber={tripItems.indexOf(item) + 1} key={item.id} compactTime={!item.time.includes(':')} />
                ))}
            </div>
          </section>
        ))}
...
function TicketStop({ item, stopNumber, compactTime }: { item: TripItem; stopNumber: number; compactTime: boolean }) {
